...
public class Responses extends AbstractModule {
    ...
    public Comments get(Long plurkId) throws RequestException {
    ...
    public JSONObject responseAdd(Long plurkId, String content, Qualifier qualifier)
        ...
        return requestBy("responseAdd").with(new Args().add("plurk_id", plurkId).add("content", content).add("qualifier", qualifier.toString())).in(HttpMethod.GET).thenJsonObject();
    }

    public JSONObject responseDelete(Long responseId, Long plurkId) throws RequestException {
        return requestBy("responseDelete").with(new Args().add("response_id", responseId).add("plurk_id", plurkId)).in(HttpMethod.GET).thenJsonObject();
    }
