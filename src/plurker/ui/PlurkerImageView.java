...
/**
 * View of an Image, intended to support the HTML tag. Supports scaling via the
 * HEIGHT and WIDTH attributes of the tag. If the image is unable to be loaded
 ...
 * @since 1.4
 */
public class PlurkerImageView extends View {
     ...
     */
    private static final String PENDING_IMAGE_SRC = "image/1344407593_wait.png";
    /**
     * File name for sMissingImageIcon.
     */
    private static final String MISSING_IMAGE_SRC = "image/1344407468_Delete.png";
    /**
     * Document property for image cache.
     */
    private static final String IMAGE_CACHE_PROPERTY = "imageCache";
// Height/width to use before we know the real size, these should at least
// the size of sMissingImageIcon and
// sPendingImageIcon
...
* @param elem the element to create a view for
     */
    public PlurkerImageView(Element elem) {
        super(elem);
    ...
    /**
     * Sets how the image is loaded. If newValue is true, the image we be loaded
     * when first asked for, otherwise it will be loaded asynchronously. The
     * default is to not load synchronously, that is to load the image
     * asynchronously.
     */
...
* @param width the width >= 0
    ...
    private Icon makeIcon(final String gifFile) throws IOException {
         ...
         */
        try {	 // replacement by V.V. to look for files in test/images directory
            //            int i = gifFile.indexOf("/") + 1;
            //            URL url = this.getClass().getClassLoader().getResource("images/" + gifFile.substring(i));
            //            ImageUtils.loadImage("./image/595fd1c19cbfa3545c4268d1c2e4056b.png");
            BufferedImage loadImage = ImageUtils.loadImage(gifFile);
            return new ImageIcon(loadImage);
        } catch (Throwable e) {
            System.err.println("?Path=" + gifFile + " " + e);
        }
        return null;
    }
    ...
    private void loadDefaultIconsIfNecessary() {
        try {
            ...
            System.err.println("myImageView: Couldn't load image icons");
        }
    }
     ...
     * instead of invoking loadImage or updateImageSize directly.
     */
...
// Only reset if this thread determined image is null
    ...
    /**
     * Invoked if no image is found, in which case a default border is used if
     * one isn't specified.
     */
...
// it will pick up the new height/width, if necessary.
        ...
        public boolean imageUpdate(Image img, int flags, int x, int y,
            ...
            if (image == null || image != img || getParent() == null) {	 // fix for OutOfMemoryError problem
                return false;
            }
...
// Bail out if there was an error:
            if ((flags & (ABORT | ERROR)) != 0) {
                repaint(0);
                synchronized (PlurkerImageView.this) {
                    if (image == img) {
